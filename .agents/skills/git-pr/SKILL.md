...
4. Create an internal commit map before staging:
   - Identify each independently reviewable product or engineering outcome.
   - Make one commit per outcome. Do not use backend/frontend boundaries as commit boundaries when they contain several independent changes.
   ...
   - A single commit is appropriate only when the whole diff tells one coherent story. Decide this yourself; do not ask the user to approve the decision.
5. For each branch or PR-sized delivery:
   ...
   e. After every commit, inspect `git status` for omitted, orphaned, or accidentally staged files before continuing.

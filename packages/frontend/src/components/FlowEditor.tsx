...
function PanelCloseButton({ label, onClick }: { label: string; onClick: () => void }) {
  return <IconButton className="editor-panel-close" label={label} icon="close" iconSize={17} onClick={onClick} />;
}
...
function Inspector({ node, nodes, edges, presets, open, update, connectOutcome, remove, close, openAgent }: { node: CanvasNode; nodes: CanvasNode[]; edges: Edge[]; presets: AgentPreset[]; open: boolean; update: (next: FlowNode) => void; connectOutcome: (outcome: string, target: string) => void; remove: () => void; close: () => void; openAgent: (presetKey: string) => void }) {
  ...
  return <aside id="flow-block-inspector" className={`inspector editor-panel ${open ? 'panel-open' : ''}`} aria-label="Block inspector">
    ...
    {flowNode.type !== 'begin' && <footer><Button variant="text" tone="danger" icon="trash" onClick={remove}>Delete block</Button></footer>}
  </aside>;
}
...
function EditorCanvas({ flowId, versionId }: { flowId: number; versionId: number | null }) {
  ...
  const publishIssueLabel = `${validation.problems.length} issue${validation.problems.length === 1 ? '' : 's'}`;
  ...
  return <div className={`editor-shell zoom-${zoomMode}${readOnly ? ' historical-view' : ''}`} data-zoom-mode={zoomMode} style={{ '--zoom-compensation': Math.min(1 / editorViewport.zoom, 1 / MIN_FLOW_ZOOM) } as CSSProperties}>
    <div className="editor-toolbar">
      <div className="editor-toolbar-context">
        <button className="back-button editor-library-action" aria-label="Flow library" onClick={() => void openLibrary()}><Icon name="back" size={15} /><span className="toolbar-nav-label">Library</span></button>
      ...
      <div className="editor-toolbar-actions">
        <Button variant="ghost" className={`editor-toolbar-control editor-history-toggle ${openPanel === 'history' ? 'active' : ''}`} icon="history" iconSize={18} aria-label="Version history" title="Version history" aria-expanded={openPanel === 'history'} aria-controls="flow-version-history" onClick={() => setOpenPanel((current) => current === 'history' ? null : 'history')}><span className="editor-toolbar-label toolbar-history-label">History</span></Button>
        ...
        {readOnly ? !displayedVersion
          ? <Button variant="primary" className="editor-toolbar-control editor-toolbar-action" data-toolbar-action="loading" icon="history" loading loadingLabel={<span className="editor-toolbar-label toolbar-action-label">Loading…</span>} />
          : selectedVersionIsCurrent
            ? <Button variant="primary" className="editor-toolbar-control editor-toolbar-action" data-toolbar-action="edit" icon="edit" onClick={() => back(flowId)}><span className="editor-toolbar-label toolbar-action-label">Edit latest draft</span></Button>
            : <Button variant="primary" className="editor-toolbar-control editor-toolbar-action" data-toolbar-action={activating ? 'activating' : 'activate'} icon="check" loading={activating} loadingLabel={<span className="editor-toolbar-label toolbar-action-label">Activating…</span>} aria-label={activating ? 'Activating' : 'Activate'} title="Use this version for future runs" onClick={() => void activateVersion()}><span className="editor-toolbar-label toolbar-action-label">Activate</span></Button>
          : <Button variant="primary" className="editor-toolbar-control editor-toolbar-action" data-toolbar-action={publishing ? 'publishing' : 'publish'} data-publish-readiness={validation.valid ? 'ready' : 'blocked'} icon="publish" loading={publishing} loadingLabel={<span className="editor-toolbar-label toolbar-action-label">Publishing…</span>} aria-label={publishing ? 'Publishing version' : 'Publish version'} aria-description={validation.valid ? undefined : `Unavailable: ${publishIssueLabel} to resolve`} title={validation.valid ? 'Publish version' : `Resolve ${publishIssueLabel} before publishing`} disabled={!validation.valid} onClick={() => void publish()}><span className="editor-toolbar-label toolbar-action-label">Publish version</span></Button>}
      </div>
    </div>
    <div className="editor-main">
      {readOnly ? <div className="editor-read-only-indicator" role="status" aria-label="Read only"><Icon name="lock" size={15} /><span>Read only</span></div> : <><div className="canvas-edit-actions"><Button variant="ghost" className={`editor-palette-toggle ${openPanel === 'palette' ? 'active' : ''}`} icon="plus" aria-expanded={openPanel === 'palette'} aria-controls="flow-block-library" onClick={() => setOpenPanel((current) => current === 'palette' ? null : 'palette')}>Blocks</Button><Button variant="ghost" className="editor-canvas-action" icon="layout" aria-label="Automatically arrange flow" title="Arrange blocks into a compact flow" onClick={autoArrange}>Arrange</Button></div><BlockPalette nodes={nodes} open={openPanel === 'palette'} add={addFromPalette} /></>}
